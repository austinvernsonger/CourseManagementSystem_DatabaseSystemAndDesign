...
/*		System.out.println("im in now");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serverMessage = ServerConsole.getSuccessfulConnectionRecordObject(username_value).getRegistrarFeedbackMessage();
		if(serverMessage.equals(Constant.RegisterCourseSuccess)){
			String payload = serverMessage + ": " + course_value;
			map.put("message", payload);
		}else if(serverMessage.equals(Constant.RegisterCourseFail_PrerequisiteConditionUnsatisfied)){
			String payload = serverMessage + ": " + course_value 
					+ "\nPlease complete these courses first: " 
					+ ServerConsole.getSuccessfulConnectionRecordObject(username_value).getPrerequisitedCourses(course_value);
			map.put("message", payload);
		}else{
			String payload = serverMessage;
			map.put("message", payload);
		}
		//map.put("message", ServerConsole.getSuccessfulConnectionRecordObject(username_value).getRegistrarFeedbackMessage());
		write(response, map);
	}
